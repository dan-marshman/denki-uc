...
def build_obj_vom_term(self):
    obj_vom = \
        ...
        / self.settings['INTERVALS_PER_HOUR']
    return obj_vom
...
def unserved_obj_fn_terms(self):
    obj_uns_demand = \
        pp.lpSum([self.settings['UNS_LOAD_PNTY']*self.vars['unserved_demand_MW'][i]
                  for i in self.sets['intervals']]) \
        / self.settings['INTERVALS_PER_HOUR']

    obj_uns_reserve = \
        pp.lpSum([self.settings['UNS_RESERVE_PNTY']*self.vars['unserved_reserve_MW'][i]
                  for i in self.sets['intervals']]) \
        / self.settings['INTERVALS_PER_HOUR']

    obj_uns_inertia = \
        pp.lpSum([self.settings['UNS_INERTIA_PNTY']*self.vars['unserved_inertia_MWsec'][i]
                  for i in self.sets['intervals']]) \
        / self.settings['INTERVALS_PER_HOUR']
