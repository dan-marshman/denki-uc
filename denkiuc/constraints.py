            ...
            for u in self.sets['units_storage']:
                ...
                condition = \
                    (self.vars['energy_in_storage_MWh'][(i, u)]
                     ...
                     + self.vars['charge_after_losses_MW'][(i, u)] * (1 / self.settings['INTERVALS_PER_HOUR'])
                     - self.vars['power_generated_MW'][(i, u)] * (1 / self.settings['INTERVALS_PER_HOUR']))
                mod += condition, label
    return mod
...
def create_constraints_df(self):
    ...
    self.constraints_df = pd.read_csv(os.path.join(self.path_to_inputs, 'constraints.csv'), index_col=0)
    return self
